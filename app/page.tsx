"use client";
// app/page.tsx — Javari Aviation
// CR AudioViz AI · EIN 39-3646201 · May 2026
import { useState } from "react";

const TOOLS = [{"icon": "\ud83d\udccb", "label": "Flight Brief", "desc": "Pre-flight briefing generator", "href": "/brief"}, {"icon": "\ud83d\udd27", "label": "Maint Log", "desc": "Maintenance entry writer", "href": "/maintenance"}, {"icon": "\ud83d\udcda", "label": "Training", "desc": "Training materials and quizzes", "href": "/training"}, {"icon": "\ud83d\udce1", "label": "NOTAM Summary", "desc": "Plain-English NOTAM summaries", "href": "/notam"}, {"icon": "\ud83d\udcdd", "label": "Incident Report", "desc": "Professional incident reports", "href": "/incident"}, {"icon": "\u2709\ufe0f", "label": "ATC Scripts", "desc": "Communication scripts", "href": "/atc"}];
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          stream: false,
          systemOverride: "You are an experienced commercial pilot and aviation professional. Provide accurate, safety-focused aviation content. Always remind users to verify with official sources.",
        }),
      });
    ...
    <div style={{ minHeight:"100vh", background:"#040912", color:"#e2e8f0", fontFamily:"system-ui" }}>
      <nav style={{ background:"#1E3A5F", padding:"0 20px", height:52, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:100 }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          <span style={{ fontWeight:800, color:"#00B4D8", fontSize:15 }}>Javari Aviation</span>
        </div>
      ...
      <section style={{ background:"linear-gradient(135deg,#1E3A5F,#040912)", padding:"56px 24px 48px", textAlign:"center" }}>
        <div style={{ maxWidth:640, margin:"0 auto" }}>
          <h1 style={{ fontSize:"clamp(24px,4vw,44px)", fontWeight:900, color:"#fff", margin:"0 0 12px", lineHeight:1.05 }}>AI Tools for<br /><span style={{ color:"#00B4D8" }}>Aviation Professionals</span></h1>
          <p style={{ color:"rgba(255,255,255,0.7)", fontSize:15, lineHeight:1.65, margin:0 }}>Flight briefings, maintenance logs, training materials, and communications for pilots and operators.</p>
        </div>
      </section>
      <section style={{ maxWidth:720, margin:"0 auto", padding:"28px 20px 0" }}>
        <div style={{ background:"#0F1F32", border:"1px solid rgba(0,180,216,0.12)", borderRadius:14, padding:"20px 24px" }}>
          <h2 style={{ margin:"0 0 12px", fontSize:15, fontWeight:700, color:"#fff" }}>Generate Flight Brief</h2>
          <div style={{ display:"flex", gap:8 }}>
            <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&generate()}
              placeholder="KRSW to KMIA, Cessna 172, afternoon departure, check weather..."
